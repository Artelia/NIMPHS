...
class NIMPHS_OT_ComputeRangesPointDataValues(Operator, NIMPHS_ModalOperator):
    """Operator to compute ranges of point data values."""

    register_cls = True
    is_custom_base_cls = False

    bl_idname = "nimphs.compute_ranges_point_data_values"
    ...
    def poll(cls, context: Context) -> bool:
        ...
        if context.scene.nimphs.m_op_running:
            return False

        if obj.nimphs.module not in ['OpenFOAM', 'TELEMAC']:
            return False
    ...
    def invoke(self, context: Context, _event: Event) -> set:
        ...
        if self.mode == 'TEST':
            self.time_point = 0
            self.end = context.scene.nimphs.file_data["ops"].nb_time_points - 1
            return {'FINISHED'}
